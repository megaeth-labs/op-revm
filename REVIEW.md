...
`MEGAETH-FORK.md` holds the rules and the update procedures; this file is the reviewer's checklist against them.
...
4. **One revm fork commit.** The `rev` in `.cargo/config.toml` is the only place that names the revm fork commit; a port PR bumps it in the same commit as the code it requires.
5. **One topic per PR, conventional prefix on every fork commit.** Commit and PR titles start with `feat`, `fix`, `chore`, `docs`, `ci` or `test` and say what changed; `git log --oneline <snapshot>..main` is the fork's changelog and must stay readable; snapshot commits are the exception and start with `Mirror op-revm@`.
