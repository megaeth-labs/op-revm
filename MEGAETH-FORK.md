...
4. **Conventional commit prefixes, one commit per topic; snapshot commits start with `Mirror op-revm@`.**
   ...
   `git diff <snapshot>..main` is the fork's footprint on upstream.
...
Bump the `rev` in `.cargo/config.toml`, fix whatever the new `Handler` shapes require in `src/handler.rs` (rule 2), and tag a release.
